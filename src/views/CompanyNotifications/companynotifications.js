...
import { CompanyNotificationsURL } from 'Redux/AdminRedux/Companynotifications/companynotificationsRedux';
...
const CompanyNotifications = () => {
    ...
    const { cmpyNotification } = useSelector((state) => state.companyNotify)
    console.log(cmpyNotification, "cmpyNotification");
    ...
    return (
        <>
            ...
            {/* List Items Start */}
            {cmpyNotification && cmpyNotification.data && cmpyNotification.data.map((item, index) => {
                return <div key="">
          ...
          <Pagination.Next className="shadow">
            ...
            <div className="d-flex justify-content-center mt-5">
                <Pagination>
                    ...
                    <Pagination.Item className="shadow" disabled={Math.ceil(cmpyNotification && cmpyNotification.count / limit) <= page + 1} onClick={() => searchfunction("page+1", page + 1)}>{page + 2}</Pagination.Item>
                    <Pagination.Item className="shadow" disabled={Math.ceil(cmpyNotification && cmpyNotification.count / limit) <= page + 2} onClick={() => searchfunction("page+2", page + 2)}>{page + 3}</Pagination.Item>

                    {Math.ceil(cmpyNotification && cmpyNotification.count / limit) > page + 3 &&
                        <>
                            <Pagination.Item className="shadow" >...</Pagination.Item>
                        </>

                    }
                    <Pagination.Next className="shadow" disabled={Math.ceil(cmpyNotification && cmpyNotification.count / limit) <= page + 1} onClick={() => searchfunction("next")}>
                        <CsLineIcons icon="chevron-right" />
                    </Pagination.Next>
                </Pagination>
            </div>
