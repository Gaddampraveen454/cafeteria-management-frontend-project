...
const category = () => {
    ...
    const HandleOffertype = (event) => {
        ...
        dispatch(AllLogincouponslist(page, search, limit, Typevalue1, event === null ? "" : event?.value, currentUser?.data?.uuid, storeUpdateDrop1, currentUser?.token))
    }

    const HandleType = (event) => {
        setTypeValue(event)
        setTypeValue1(event?.value)
        dispatch(AllLogincouponslist(page, search, limit, event === null ? "" : event?.value, offerTypevalue1, currentUser?.data?.uuid, storeUpdateDrop1, currentUser?.token))

    }

    useEffect(() => {
        dispatch(AllLogincouponslist(page, search, limit, Typevalue1, offerTypevalue1, currentUser?.data?.uuid, storeUpdateDrop1, currentUser?.token))
    ...
    const searchfunction = (type, pages) => {
        if (type === "search") {
            ...
            dispatch(AllLogincouponslist(0, page, limit, Typevalue1, offerTypevalue1, currentUser?.data?.uuid, storeUpdateDrop1, currentUser?.token))
        }
        if (type === "prev") {
            setPage(page - 1)
            // dispatch(AdminCategoryListURL(page - 1, search, limit, currentUser?.data?.uuid, storeDrop))
            dispatch(AllLogincouponslist(page - 1, search, limit, Typevalue1, offerTypevalue1, currentUser?.data?.uuid, storeUpdateDrop1, currentUser?.token))
        }
        else if (type === "next") {
            setPage(page + 1)
            // dispatch(AdminCategoryListURL(page + 1, search, limit, currentUser?.data?.uuid, storeDrop))
            ...
            dispatch(AllLogincouponslist(page + 2, search, limit, Typevalue1, offerTypevalue1, currentUser?.data?.uuid, storeUpdateDrop1, currentUser?.token))
            ...
            dispatch(AllLogincouponslist(0, search, pages, Typevalue1, offerTypevalue1, currentUser?.data?.uuid, storeUpdateDrop1, currentUser?.token))
    ...
    useEffect(() => {
        dispatch(ICafeAdminCategoryStoreDropDownList(currentUser?.data?.uuid));
    }, [])
    ...
    const handleUpdateStore = (selected) => {
        ...
        dispatch(AllLogincouponslist(page, search, limit, Typevalue1, offerTypevalue1, currentUser?.data?.uuid, selected?.value, currentUser?.token))
    }
