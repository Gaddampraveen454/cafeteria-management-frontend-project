...
import { CompanyConsumerUpdateURL, CompanyConsumerBulkUploadURL, CompanyConsumerStatusUpdateURL } from 'Redux/AdminRedux/Consumer/ConsumerRedux';
...
const role = () => {
    ...
    const title = 'Company Role List';
    const description = 'Ecommerce User Management Page';

    const [openPopup, setOpenPopup] = React.useState(false);
    const [eventType, setEventType] = useState(false)
    const { currentUser } = useSelector((state) => state.auth)
    // const {companyRoleData,notification} = useSelector((state)=>state.userManagement);
    // console.log(companyRoleData,'eghverv')
    // const {  notification } = useSelector((state) => state.comapnuserSlice)
    // console.log(companyRoleData, 'vdghvehgfvhegvfv')
    ...
    // useEffect(()=>{
    //     dispatch(CompanyRoleListURL( currentUser.token))
    // }, [])
    ...
    // console.log(companyRoleData, "cashierDatadassadad")
    ...
    return (
        <>
          ...
          {"Use Google's location service?"}
            ...
            {companyRoleData?.data?.length > 0 && companyRoleData?.data?.map((item, index) => {
                console.log(item, 'hevghevfgy')
            ...
            <div className="d-flex justify-content-center mt-5">
                <Pagination>
                    ...
                    <Pagination.Item className="shadow" disabled={Math.ceil(companyRoleData && companyRoleData.count / limit) <= page + 1} onClick={() => searchfunction("page+1", page + 1)}>{page + 2}</Pagination.Item>
                    <Pagination.Item className="shadow" disabled={Math.ceil(companyRoleData && companyRoleData.count / limit) <= page + 2} onClick={() => searchfunction("page+2", page + 2)}>{page + 3}</Pagination.Item>

                    {Math.ceil(companyRoleData && companyRoleData.count / limit) > page + 3 &&
                        <>
                            <Pagination.Item className="shadow" >...</Pagination.Item>
                        </>

                    }
                    <Pagination.Next className="shadow" disabled={Math.ceil(companyRoleData && companyRoleData.count / limit) <= page + 1} onClick={() => searchfunction("next")}>
