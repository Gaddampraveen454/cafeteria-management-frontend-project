...
const ProductForConsumerSlice = createSlice({
  name: 'products',
  initialState,
  reducers: {
    setProductForConsumer(state, action) {
      state.ProductForConsumer = action.payload;
    },
...
export const ProductForConsumerListURL = (companyId, catagory, page, search, token, limit, storeSlug, type) => async (dispatch) => {
  const response = await axios.get(`${process.env.REACT_APP_URL}/product/lists?pagenum=${page}&limit=${limit}&company_slug=&store_slug=${storeSlug}&category_uuid=${catagory}&search=${search}&type=${type}`, {
    // headers: {
    //   "x-auth-token": token
    // }
  }).then((res) => {
    console.log(res, "sdfsdfsdsdfsdfsdfsdfff")
    dispatch(setProductForConsumer(res.data));
  })
    .catch((err) => {
      console.log("err");
    })
...
// };
const ProductForConsumerReducer = ProductForConsumerSlice.reducer;

export default ProductForConsumerReducer;
