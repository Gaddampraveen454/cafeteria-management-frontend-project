...
export const AllLogincouponslist = (page, search, limit, type, offertype, companyuuid, storeuuid, token) => async (dispatch) => {
    const response = await axios.get(`${process.env.REACT_APP_URL}/coupon/list?page=${page}&limit=${limit}&search=${search}&type=${type}&offer_type=${offertype}&company_uuid=${companyuuid}&store_uuid=${storeuuid}`, {
        headers: {
            "x-auth-token": token
        }
    }).then((res) => {
...
export const AllLoginAddCoupon = (payload, token) => async (dispatch) => {
...
export const AllLoginUpdateCoupon = (uuid, payload, token) => async (dispatch) => {
