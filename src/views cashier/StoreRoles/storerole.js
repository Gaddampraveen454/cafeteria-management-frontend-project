...
import {  ProductAddURL, StoreProductUpdateURL, StoreProductBulkUplodURL, StoreProductStatusUpdateURL } from 'Redux/CashierRedux/Product/ProductRedux';
import { toast } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';
import axios from 'axios';
import { StoreRoleListURL } from 'Redux/CashierRedux/StoreRoleredux/storeroleredux';

const storerole = () => {
  const dispatch = useDispatch()
  const title = 'Store Role List';
  ...
  const {storeRoleData,notification}=useSelector((state)=>state.StoreRoleSlice);
  console.log(storeRoleData,'vshghfrhvf');
...
//   const {  storeRoleData, notification } = useSelector((state) => state.StoreproductSlice)
  const { categoryDropdown } = useSelector((state) => state.StorecategorySlice)
  useEffect(() => {
    dispatch( StoreRoleListURL(page, search, currentUser.token, limit,group))
    dispatch(StoreCategoryDropDownL(currentUser?.data?.uuid))
  }, [selectcat1])
//   useEffect(()=>{
//    dispatch(StoreRoleListURL(currentUser?.token))
//   },[])
  useEffect(() => {
    if (suc === true) {
  ...
  const searchfunction = (type, pages) => {
    if (type === "search") {
      setSearch(pages)
      setPage(0)
      dispatch( StoreRoleListURL(0, pages, currentUser.token, limit,group))
    }
    if (type === "prev") {
      setPage(page - 1)
      dispatch( StoreRoleListURL(page - 1, search, currentUser.token, limit,group))
    }
    else if (type === "next") {
      setPage(page + 1)
      dispatch( StoreRoleListURL(page + 1, search, currentUser.token, limit,group))
      ...
      setPage(page + 2)
      dispatch( StoreRoleListURL(page + 2, search, currentUser.token, limit,group))
    }
    else if (type === "limit") {
      setLimit(pages)
      setPage(0)
      dispatch( StoreRoleListURL(0, search, currentUser.token, pages,group))
    }
  }
...
const handleGroup = (selectValue) => {
    ...
    dispatch( StoreRoleListURL(page, search, currentUser.token, limit,selectValue === null ? "":selectValue?.value))
  ...
  return (
    <>
      ...
      <Row className="g-0 mb-2 d-none d-lg-flex">
        ...
        <Col>
          <Row className="g-0 h-100 align-content-center custom-sort ps-5 pe-4 h-100">
            ...
            <Col xs="1" lg="1" className="d-flex flex-column pe-1 justify-content-center">
              <div className="text-muted text-medium cursor-pointer sort">group</div>
            </Col>
      ...
      {/* List Items Start */}
      { storeRoleData &&  storeRoleData.data &&  storeRoleData.data.map((item, index) => {
        console.log(item,'dvcsdvcgdvcsdvcg')
        return <div key="">
      ...
      <div className="d-flex justify-content-center mt-5">
        <Pagination>
          ...
          <Pagination.Item className="shadow" disabled={Math.ceil( storeRoleData &&  storeRoleData.count / limit) <= page + 1} onClick={() => searchfunction("page+1", page + 1)}>{page + 2}</Pagination.Item>
          <Pagination.Item className="shadow" disabled={Math.ceil( storeRoleData &&  storeRoleData.count / limit) <= page + 2} onClick={() => searchfunction("page+2", page + 2)}>{page + 3}</Pagination.Item>

          {Math.ceil( storeRoleData &&  storeRoleData.count / limit) > page + 3 &&
            <>
              <Pagination.Item className="shadow" >...</Pagination.Item>
            </>

          }
          <Pagination.Next className="shadow" disabled={Math.ceil( storeRoleData &&  storeRoleData.count / limit) <= page + 1} onClick={() => searchfunction("next")}>
            <CsLineIcons icon="chevron-right" />
          </Pagination.Next>
        </Pagination>
      </div>
