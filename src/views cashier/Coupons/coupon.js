...
import { AllLogincouponslist, companyCouponStatus } from 'Redux/IcafeAdminRedux/CouponsRedux/coponscompanyredux';
...
const category = () => {
    ...
    console.log(currentUser?.data?.company_uuid, 'dfghfghvcgvdh')
    // /CouponList
    ...
    const [offerTypevalue1, setOfferTypeValue1] = useState('');
    ...
    const HandleOffertype = (event) => {
        console.log(event, "hjgfdjhdfkjhcdskjf")
        setOfferTypeValue(event)
        setOfferTypeValue1(event?.value)
        dispatch(AllLogincouponslist(page, search, limit, Typevalue1, event === null ? "" : event?.value, currentUser?.data?.company_uuid, currentUser?.data?.uuid, currentUser?.token))
    }

    const HandleType = (event) => {
        setTypeValue(event)
        setTypeValue1(event?.value)
        dispatch(AllLogincouponslist(page, search, limit, event === null ? "" : event?.value, offerTypevalue1, currentUser?.data?.company_uuid, currentUser?.data?.uuid, currentUser?.token))

    }

    useEffect(() => {
        dispatch(AllLogincouponslist(page, search, limit, Typevalue1, offerTypevalue1, currentUser?.data?.company_uuid, currentUser?.data?.uuid, currentUser?.token))
    ...
    const searchfunction = (type, pages) => {
        if (type === "search") {
            ...
            dispatch(AllLogincouponslist(0, pages, limit, Typevalue1, offerTypevalue1, currentUser?.data?.company_uuid, currentUser?.data?.uuid, currentUser?.token))
        }
        if (type === "prev") {
            setPage(page - 1)
            // dispatch(AdminCategoryListURL(page - 1, search, limit, currentUser?.data?.uuid, storeDrop))
            dispatch(AllLogincouponslist(page - 1, search, limit, Typevalue1, offerTypevalue1, currentUser?.data?.company_uuid, currentUser?.data?.uuid, currentUser?.token))
        }
        else if (type === "next") {
            setPage(page + 1)
            // dispatch(AdminCategoryListURL(page + 1, search, limit, currentUser?.data?.uuid, storeDrop))
            ...
            dispatch(AllLogincouponslist(page + 2, search, limit, Typevalue1, offerTypevalue1, currentUser?.data?.company_uuid, currentUser?.data?.uuid, currentUser?.token))
            ...
            dispatch(AllLogincouponslist(0, search, pages, Typevalue1, offerTypevalue1, currentUser?.data?.company_uuid, currentUser?.data?.uuid, currentUser?.token))
    ...
    const selectCompanyFunction = (selectedEvent) => {
        ...
        dispatch(AllLogincouponslist(page, search, limit, Typevalue1, offerTypevalue1, selectedEvent === null ? "" : selectedEvent?.value, "", currentUser?.token))
    }
    ...
    const handleUpdateDrop = (select) => {
        ...
        dispatch(AllLogincouponslist(page, search, limit, Typevalue1, offerTypevalue1, select?.value, "", currentUser?.token))
    }

    const handleUpdateStore = (selected) => {
        setStoreUpdateDrop(selected)
        dispatch(AllLogincouponslist(page, search, limit, Typevalue1, offerTypevalue1, companyUpdateDrop?.value, selected?.value, currentUser?.token))
    }
        ...
        if (suc === true) {
            if (notification.status === true) {
                ...
                setTimeout(() => {
                    ...
                    dispatch(AllLogincouponslist(page, search, limit, Typevalue1, offerTypevalue1, companyUpdateDrop, storeUpdateDrop, currentUser?.token))
