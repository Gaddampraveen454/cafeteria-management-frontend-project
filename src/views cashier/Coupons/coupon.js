...
const category = () => {
    ...
    useEffect(() => {
        dispatch(companycouponslist(page, search, limit, "", currentUser?.data?.company_uuid, currentUser?.data?.uuid, currentUser?.token))
    ...
    const searchfunction = (type, pages) => {
        if (type === "search") {
            ...
            dispatch(companycouponslist(0, pages, limit, "", currentUser?.data?.company_uuid, currentUser?.data?.uuid, currentUser?.token))
        }
        if (type === "prev") {
            setPage(page - 1)
            // dispatch(AdminCategoryListURL(page - 1, search, limit, currentUser?.data?.uuid, storeDrop))
            dispatch(companycouponslist(page - 1, search, limit, "", currentUser?.data?.company_uuid, currentUser?.data?.uuid, currentUser?.token))
        }
        else if (type === "next") {
            setPage(page + 1)
            // dispatch(AdminCategoryListURL(page + 1, search, limit, currentUser?.data?.uuid, storeDrop))
            ...
            dispatch(companycouponslist(page + 2, search, limit, "", currentUser?.data?.company_uuid, currentUser?.data?.uuid, currentUser?.token))
            ...
            dispatch(companycouponslist(0, search, pages, "",currentUser?.data?.company_uuid, currentUser?.data?.uuid, currentUser?.token))
    ...
    const handleUpdateStore = (selected) => {
        ...
        dispatch(companycouponslist(page, search, limit, "", companyUpdateDrop?.value, selected?.value, currentUser?.token))
    }
        ...
        if (suc === true) {
            if (notification.status === true) {
                ...
                setTimeout(() => {
                    ...
                    dispatch(companycouponslist(page, search, limit, "", companyUpdateDrop, storeUpdateDrop, currentUser?.token))
