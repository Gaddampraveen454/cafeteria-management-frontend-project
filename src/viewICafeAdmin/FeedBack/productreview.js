...
import { ICafeAdminProductViewURL } from 'Redux/IcafeAdminRedux/Feedbackredux/feedbackdux';
...
const productreview = () => {
    ...
    useEffect(()=>{
        dispatch(ICafeAdminProductViewURL(page,search,currentUser.token,limit,location?.state?.product_uuid,ratingval))
    },[])
    ...
    const searchfunction = (type, pages) => {
        if (type === "search") {
            ...
            setSearch(pages)
            setPage(0)
            dispatch(ICafeAdminProductViewURL(0, pages, currentUser.token, limit,location?.state?.product_uuid,ratingval))
        }
        if (type === "prev") {
            setPage(page - 1)
            dispatch(ICafeAdminProductViewURL(page - 1, search, currentUser.token, limit,location?.state?.product_uuid,ratingval))
        }
        else if (type === "next") {
            setPage(page + 1)
            dispatch(ICafeAdminProductViewURL(page + 1, search, currentUser.token, limit,location?.state?.product_uuid,ratingval))
            ...
            setPage(page + 2)
            dispatch(ICafeAdminProductViewURL(page + 2, search, currentUser.token, limit,location?.state?.product_uuid,ratingval))
        }
        else if (type === "limit") {
            setLimit(pages)
            setPage(0)
            dispatch(ICafeAdminProductViewURL(0, search, currentUser.token, pages,location?.state?.product_uuid,ratingval))
        }
    }
    ...
    const ratingChange=(value)=>{
     ...
     dispatch(ICafeAdminProductViewURL(page,search,currentUser.token,limit,location?.state?.product_uuid,value?.value))
    ...
    return (
        ...
        <Col md="1" className="d-flex flex-column pe-1 justify-content-center">
            ...
            {/* List Items Start */}
            {ProductView?.data?.length > 0 && ProductView?.data?.map((text, index) => {
            ...
            <div className="d-flex justify-content-center mt-5">
                <Pagination>
                    ...
                    <Pagination.Item className="shadow" disabled={Math.ceil(ProductView && ProductView.count / limit) <= page + 1} onClick={() => searchfunction("page+1", page + 1)}>{page + 2}</Pagination.Item>
                    <Pagination.Item className="shadow" disabled={Math.ceil(ProductView && ProductView.count / limit) <= page + 2} onClick={() => searchfunction("page+2", page + 2)}>{page + 3}</Pagination.Item>

                    {Math.ceil(ProductView && ProductView.count / limit) > page + 3 &&
                        <>
                            <Pagination.Item className="shadow" >...</Pagination.Item>
                        </>

                    }
                    <Pagination.Next className="shadow" disabled={Math.ceil(ProductView && ProductView.count / limit) <= page + 1} onClick={() => searchfunction("next")}>
