...
import { AdminNotificationsURL } from 'Redux/IcafeAdminRedux/AdminNotifications/adminnotificationsredux'
...
const AdminNotifications = () => {
    ...
    const { adminNotification } = useSelector((state) => state.adminNotify)
    console.log(adminNotification, "adminNotification");


    useEffect(() => {
        dispatch(AdminNotificationsURL(page, search, currentUser.token, limit, currentUser?.data?.uuid))
    }, [])
    ...
    const searchfunction = (type, pages) => {
        ...
        if (type === "search") {
            ...
            setSearch(pages)
            setPage(0)
            dispatch(AdminNotificationsURL(0, pages, currentUser.token, limit, currentUser?.data?.uuid))
        }
        if (type === "prev") {
            setPage(page - 1)
            dispatch(AdminNotificationsURL(page - 1, search, currentUser.token, limit, currentUser?.data?.uuid))
        }
        else if (type === "next") {
            setPage(page + 1)
            dispatch(AdminNotificationsURL(page + 1, search, currentUser.token, limit, currentUser?.data?.uuid))
            ...
            setPage(page + 2)
            dispatch(AdminNotificationsURL(page + 2, search, currentUser.token, limit, currentUser?.data?.uuid))
        }
        else if (type === "limit") {
            setLimit(pages)
            setPage(0)
            dispatch(AdminNotificationsURL(0, search, currentUser.token, pages, currentUser?.data?.uuid))
        }
    }
    ...
    return (
        <>
            ...
            {/* List Items Start */}
            {adminNotification && adminNotification.data && adminNotification.data.map((item, index) => {
                return <div key="">
                    {console.log(item, "dffdfdfdfsssfsdfsdf")}
                    <Card className={`mb-2 ${selectedItems.includes(1) && 'selected'}`}>
                        <Row className="g-0 h-100 sh-lg-9 position-relative">
                            <NavLink to={`/companyViewOrder/${item?.link}`}>
          ...
          <Pagination.Next className="shadow">
            ...
            <div className="d-flex justify-content-center mt-5">
                <Pagination>
                    ...
                    <Pagination.Item className="shadow" disabled={Math.ceil(adminNotification && adminNotification.count / limit) <= page + 1} onClick={() => searchfunction("page+1", page + 1)}>{page + 2}</Pagination.Item>
                    <Pagination.Item className="shadow" disabled={Math.ceil(adminNotification && adminNotification.count / limit) <= page + 2} onClick={() => searchfunction("page+2", page + 2)}>{page + 3}</Pagination.Item>

                    {Math.ceil(adminNotification && adminNotification.count / limit) > page + 3 &&
                        <>
                            <Pagination.Item className="shadow" >...</Pagination.Item>
                        </>

                    }
                    <Pagination.Next className="shadow" disabled={Math.ceil(adminNotification && adminNotification.count / limit) <= page + 1} onClick={() => searchfunction("next")}>
                        <CsLineIcons icon="chevron-right" />
                    </Pagination.Next>
                </Pagination>
            </div>
