...
import { iCafeAdminConsumerListURL, consumerAddURL, iCafeAdminConsumerUpdateURL, iCafeAdminConsumerBulkUploadURL, iCafeAdminConsumerStatusUpdateURL } from 'Redux/IcafeAdminRedux/ICafeAdminUserManagement/adminuserredux';
...
const AdminUser = () => {
  ...
  const { consumerData, notification } = useSelector((state) => state.iacfeadminconsumer)
  ...
  const UpdateConsumer = (event) => {
    ...
    dispatch(iCafeAdminConsumerUpdateURL(consmerId, payload, currentUser.token))
    setSuc(true)
  ...
  function handleSubmit(event) {
    if (!file) {
      ...
      dispatch(iCafeAdminConsumerBulkUploadURL(formData, currentUser.token))
      setSuc(true)
    }
  }
  ...
  const HandleUserStatus = (event) => {
    ...
    dispatch(iCafeAdminConsumerStatusUpdateURL(payload, currentUser.token))
    setSuc(true)
