...
const category = () => {
    ...
    const [companyUpdateDrop1, setCompanyUpdateDrop1] = useState('');

    const [storeUpdateDrop, setStoreUpdateDrop] = useState('');
    const [storeUpdateDrop1, setStoreUpdateDrop1] = useState('');
    ...
    const HandleOffertype = (event) => {
        ...
        dispatch(AllLogincouponslist(page, search, limit, Typevalue1, event === null ? "" : event?.value, companyUpdateDrop1, storeUpdateDrop1, currentUser?.token))
    }

    const HandleType = (event) => {
        setTypeValue(event)
        setTypeValue1(event?.value)
        dispatch(AllLogincouponslist(page, search, limit, event === null ? "" : event?.value, offerTypevalue1, companyUpdateDrop1, storeUpdateDrop1, currentUser?.token))

    }

    useEffect(() => {
        dispatch(AllLogincouponslist(page, search, limit, Typevalue1, offerTypevalue1, companyUpdateDrop1, storeUpdateDrop1, currentUser?.token))
        // dispatch(AllLogincouponslist())
    }, [])

    const eventHandler = (event) => {
        console.log(event, "eventevent")
        history.push({
            pathname: "/Update-Coupon",
            state: event
        })
    }
    ...
    const updateCategory = (event) => {
        ...
        const payload = {
            "company_uuid": companyUpdateDrop1,
            "store_uuid": storeUpdateDrop1,
            "name": name,
    ...
    const searchfunction = (type, pages) => {
        if (type === "search") {
            ...
            dispatch(AllLogincouponslist(0, pages, limit, Typevalue1, offerTypevalue1, companyUpdateDrop1, storeUpdateDrop1, currentUser?.token))
        }
        if (type === "prev") {
            setPage(page - 1)
            // dispatch(AdminCategoryListURL(page - 1, search, limit, currentUser?.data?.uuid, storeDrop))
            dispatch(AllLogincouponslist(page - 1, search, limit, Typevalue1, offerTypevalue1, companyUpdateDrop1, storeUpdateDrop1, currentUser?.token))
        }
        else if (type === "next") {
            setPage(page + 1)
            // dispatch(AdminCategoryListURL(page + 1, search, limit, currentUser?.data?.uuid, storeDrop))
            ...
            dispatch(AllLogincouponslist(page + 2, search, limit, Typevalue1, offerTypevalue1, companyUpdateDrop1, storeUpdateDrop1, currentUser?.token))
            ...
            dispatch(AllLogincouponslist(0, search, pages, Typevalue1, offerTypevalue1, companyUpdateDrop1, storeUpdateDrop1, currentUser?.token))
    ...
    const handleUpdateStore = (selected) => {
        ...
        setStoreUpdateDrop1(selected?.value)
        dispatch(AllLogincouponslist(page, search, limit, Typevalue1, offerTypevalue1, companyUpdateDrop1, selected?.value, currentUser?.token))
    }
    ...
    return (
        <>
            ...
            <Row className="mb-3">
                ...
                <Col lg="2" className="mb-1 text-end">
